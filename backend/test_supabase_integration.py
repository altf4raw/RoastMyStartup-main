    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if result['database'] == 'healthy':
                ...
                print("⚠️ Database connection unavailable (expected with placeholder credentials)")
        else:
...
def test_roast_with_database_persistence():
    ...
    try:
        ...
        if response.status_code == 200:
            result = response.json()
            print(f"✅ Roast generated successfully!")
            ...
            print("✅ User received roast regardless of database status (fail-safe working)")
        else:
